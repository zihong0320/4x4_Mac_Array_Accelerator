...
- Data reuse를 최대한 사용하기 위해 PE(processing element)를 사용하여 연산하는 부분을 구성함
...
- 해결책 (개선된 구조):
     - 1개 PE 내에 2개의 Weight를 저장하는 구조와 Weight Stationary 기법을 적용
     - 한 번의 메모리 접근(64비트)으로 8개의 Weight를 각 MAC에 미리 저장함으로써 Control Unit의 상태(State)를 줄이고 전체 Dataflow를 단순화
...
- 이를 64비트 버스로 동시에 정렬하여 출력하기 위해 하단에 동기화 레지스터를 배치하여 타이밍을 무조건 Cycle 7로 통일
